  ...
  return (
    <div className={'min-h-[12.375rem] rounded-xl bg-[#F6F6F6] p-2'}>
      <Card className={'flex h-full flex-col px-3'}>
        <CardHeader className={'pb-1 text-lg font-semibold'}>Assigned Cases by User</CardHeader>
        <CardContent>
          <ul className={'w-full max-w-sm'}>
            {assignees?.map(({ name, value }) => {
              return (
                <li key={name} className={'flex items-center justify-between space-x-4 text-base'}>
